...
describe("computeCanvasSize", () => {
  it("枠・役割が無くても最小サイズを返す(0除算等をしない)", () => {
    const { width, height } = computeCanvasSize(project(), {});
    expect(width).toBeGreaterThan(0);
    expect(height).toBeGreaterThan(0);
  });

  it("枠が増えるほど縦に、役割が増えるほど横に大きくなる", () => {
    const base = computeCanvasSize(project(), {});
  ...
  it("subtitleがあると縦に少し大きくなる", () => {
    const withoutSubtitle = computeCanvasSize(project(), {});
    const withSubtitle = computeCanvasSize(project(), { subtitle: "2026-08-30" });
